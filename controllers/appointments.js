...
exports.bookAppointment = async (req, res, next) => {
  try {
    ...
    // Check for existing appointment at the same time (only for same doctor)
    ...
    let appointmentStatus = "pending"; // Default for patient bookings
    if (req.user.role === "doctor") {
      appointmentStatus = "confirmed"; // Doctor bookings are always confirmed
    } else if (req.body.status) {
      appointmentStatus = req.body.status; // Allow explicit status setting
    }
    ...
    // Update doctor availability if doctor document exists
    const doctorDoc = await Doctor.findOne({ user: doctorUserId });
    if (doctorDoc) {
